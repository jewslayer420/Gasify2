...
const { fetchNorthMacedoniaStations } = require('../services/scrapers/northmacedonia');
const prisma = require('../lib/prisma');
const CHUNK = 2000;
async function run() {
  const oldIds = (await prisma.station.findMany({ where: { country: { in: ['MK', 'XK'] } }, select: { id: true } })).map(s => s.id);
  if (oldIds.length) {
    ...
    console.log(`[script] Deleted ${oldIds.length} old MK/XK stations`);
  }
  const stations = await fetchNorthMacedoniaStations();
  console.log(`[script] Fetched ${stations.length} North Macedonia/Kosovo stations`);
  let totalPrices = 0;
