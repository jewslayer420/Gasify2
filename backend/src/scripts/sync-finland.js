...
const { fetchFinlandStations } = require('../services/scrapers/finland');
...
async function run() {
  ...
  console.log(`[script] ${coordsCache.size} cached FI station coords`);
  ...
  if (oldIds.length) {
    ...
    console.log(`[script] Deleted ${oldIds.length} old FI stations`);
  }

  const stations = await fetchFinlandStations(coordsCache);
  console.log(`[script] Fetched ${stations.length} Finland stations`);
