...
const { fetchAlbaniaStations } = require('../services/scrapers/albania');
const prisma = require('../lib/prisma');
const CHUNK = 2000;
async function run() {
  const oldIds = (await prisma.station.findMany({ where: { country: 'AL' }, select: { id: true } })).map(s => s.id);
  if (oldIds.length) {
    ...
    console.log(`[script] Deleted ${oldIds.length} old AL stations`);
  }
  const stations = await fetchAlbaniaStations();
  console.log(`[script] Fetched ${stations.length} Albania stations`);
  let totalPrices = 0;
