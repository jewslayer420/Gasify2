...
const { fetchBosniaStations } = require('../services/scrapers/bosnia');
const prisma = require('../lib/prisma');
const CHUNK = 2000;
async function run() {
  const oldIds = (await prisma.station.findMany({ where: { country: 'BA' }, select: { id: true } })).map(s => s.id);
  if (oldIds.length) {
    ...
    console.log(`[script] Deleted ${oldIds.length} old BA stations`);
  }
  const stations = await fetchBosniaStations();
  console.log(`[script] Fetched ${stations.length} Bosnia stations`);
  let totalPrices = 0;
