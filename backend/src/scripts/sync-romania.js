...
const { fetchRomaniaStations } = require('../services/scrapers/romania');
const prisma = require('../lib/prisma');
const CHUNK = 2000;
async function run() {
  const oldIds = (await prisma.station.findMany({ where: { country: 'RO' }, select: { id: true } })).map(s => s.id);
  if (oldIds.length) {
    ...
    console.log(`[script] Deleted ${oldIds.length} old RO stations`);
  }
  const stations = await fetchRomaniaStations();
  console.log(`[script] Fetched ${stations.length} Romania stations`);
  let totalPrices = 0;
