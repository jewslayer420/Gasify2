...
const { fetchMexicoStations } = require('../services/scrapers/mexico');
const prisma = require('../lib/prisma');

const CHUNK = 500;

async function run() {
  const stations = await fetchMexicoStations();
  console.log(`[script] Fetched ${stations.length} Mexico stations`);
