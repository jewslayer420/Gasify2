...
const { fetchTaiwanStations } = require('../services/scrapers/taiwan');
const prisma = require('../lib/prisma');

const CHUNK = 500;

async function run() {
  const stations = await fetchTaiwanStations();
  console.log(`[script] Fetched ${stations.length} Taiwan stations`);
