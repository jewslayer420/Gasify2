...
const { fetchDenmarkStations } = require('../services/scrapers/denmark');
const prisma = require('../lib/prisma');

const CHUNK = 500;

async function run() {
  const stations = await fetchDenmarkStations();
  console.log(`[script] Fetched ${stations.length} Denmark stations`);
