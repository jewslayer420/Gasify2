...
const { fetchThailandStations } = require('../services/scrapers/thailand');
const prisma = require('../lib/prisma');

const CHUNK = 500;

async function run() {
  const stations = await fetchThailandStations();
  console.log(`[script] Fetched ${stations.length} Thailand stations`);
