...
const { fetchNLStations } = require('../services/scrapers/netherlands');
const prisma = require('../lib/prisma');
const CHUNK = 2000;
async function run() {
  const oldIds = (await prisma.station.findMany({ where: { country: 'NL' }, select: { id: true } })).map(s => s.id);
  if (oldIds.length) {
    ...
    console.log(`[script] Deleted ${oldIds.length} old NL stations`);
  }
  const stations = await fetchNLStations();
  console.log(`[script] Fetched ${stations.length} Netherlands stations`);
  let totalPrices = 0;
