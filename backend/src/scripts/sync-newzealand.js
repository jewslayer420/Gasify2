...
const { fetchNewZealandStations } = require('../services/scrapers/newzealand');
const prisma = require('../lib/prisma');

const CHUNK = 500;

async function run() {
  const stations = await fetchNewZealandStations();
  console.log(`[script] Fetched ${stations.length} New Zealand stations`);
