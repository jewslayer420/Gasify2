...
const { fetchVICStations } = require('../services/scrapers/australia_vic');
const prisma = require('../lib/prisma');

const CHUNK = 500;

async function run() {
  const stations = await fetchVICStations();
  console.log(`[script] Fetched ${stations.length} VIC stations`);
