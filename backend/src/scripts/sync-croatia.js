...
const { fetchCroatiaStations } = require('../services/scrapers/croatia');
const prisma = require('../lib/prisma');
const CHUNK = 2000;
async function run() {
  const oldIds = (await prisma.station.findMany({ where: { country: 'HR' }, select: { id: true } })).map(s => s.id);
  if (oldIds.length) {
    ...
    console.log(`[script] Deleted ${oldIds.length} old HR stations`);
  }
  const stations = await fetchCroatiaStations();
  console.log(`[script] Fetched ${stations.length} Croatia stations`);
  let totalPrices = 0;
