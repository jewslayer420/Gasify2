...
const { fetchCanadaStations } = require('../services/scrapers/canada');
const prisma = require('../lib/prisma');

const CHUNK = 500;

async function run() {
  const stations = await fetchCanadaStations();
  console.log(`[script] Fetched ${stations.length} Canada stations`);
