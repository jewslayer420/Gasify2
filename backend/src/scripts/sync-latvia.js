...
const { fetchLatviaStations } = require('../services/scrapers/latvia');
const prisma = require('../lib/prisma');

const CHUNK = 500;

async function run() {
  const existing = await prisma.station.findMany({ where: { country: 'LV' }, select: { id: true } });
  ...
  if (oldIds.length) {
    ...
    console.log(`[script] Deleted ${oldIds.length} old LV stations`);
  }

  const stations = await fetchLatviaStations();
  console.log(`[script] Fetched ${stations.length} Latvia stations`);
