...
const { fetchSpainStations } = require('../services/scrapers/spain');
const prisma = require('../lib/prisma');
const CHUNK = 2000;
async function run() {
  const oldIds = (await prisma.station.findMany({ where: { country: 'ES' }, select: { id: true } })).map(s => s.id);
  if (oldIds.length) {
    ...
    console.log(`[script] Deleted ${oldIds.length} old ES stations`);
  }
  const stations = await fetchSpainStations();
  console.log(`[script] Fetched ${stations.length} Spain stations`);
  let totalPrices = 0;
