...
const { fetchBulgariaStations } = require('../services/scrapers/bulgaria');
const prisma = require('../lib/prisma');
const CHUNK = 2000;
async function run() {
  const oldIds = (await prisma.station.findMany({ where: { country: 'BG' }, select: { id: true } })).map(s => s.id);
  if (oldIds.length) {
    ...
    console.log(`[script] Deleted ${oldIds.length} old BG stations`);
  }
  const stations = await fetchBulgariaStations();
  console.log(`[script] Fetched ${stations.length} Bulgaria stations`);
  let totalPrices = 0;
