...
const { fetchIrelandStations } = require('../services/scrapers/ireland');
...
async function run() {
  ...
  console.log(`[script] ${coordsCache.size} cached IE station coords`);
  ...
  if (oldIds.length) {
    ...
    console.log(`[script] Deleted ${oldIds.length} old IE stations`);
  }

  const stations = await fetchIrelandStations(coordsCache);
  console.log(`[script] Fetched ${stations.length} Ireland stations`);
