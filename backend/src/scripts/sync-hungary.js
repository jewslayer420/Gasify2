...
const { fetchHungaryStations } = require('../services/scrapers/hungary');
const prisma = require('../lib/prisma');

const CHUNK = 2000;

async function run() {
  console.log('[script] Clearing old HU stations...');
  const oldIds = (await prisma.station.findMany({ where: { country: 'HU' }, select: { id: true } })).map(s => s.id);
  if (oldIds.length) {
    ...
    console.log(`[script] Deleted ${oldIds.length} old HU stations`);
  }

  const stations = await fetchHungaryStations();
  console.log(`[script] Fetched ${stations.length} Hungary stations`);
