...
const { fetchSerbiaStations } = require('../services/scrapers/serbia');
const prisma = require('../lib/prisma');
const CHUNK = 2000;
async function run() {
  const oldIds = (await prisma.station.findMany({ where: { country: 'RS' }, select: { id: true } })).map(s => s.id);
  if (oldIds.length) {
    ...
    console.log(`[script] Deleted ${oldIds.length} old RS stations`);
  }
  const stations = await fetchSerbiaStations();
  console.log(`[script] Fetched ${stations.length} Serbia stations`);
  let totalPrices = 0;
