...
const { fetchSwedenStations } = require('../services/scrapers/sweden');
...
async function run() {
  ...
  console.log(`[script] ${coordsCache.size} cached SE station coords`);
  ...
  if (oldIds.length) {
    ...
    console.log(`[script] Deleted ${oldIds.length} old SE stations`);
  }

  const stations = await fetchSwedenStations(coordsCache);
  console.log(`[script] Fetched ${stations.length} Sweden stations`);
