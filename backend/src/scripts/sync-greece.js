...
const { fetchGreeceStations } = require('../services/scrapers/greece');
const prisma = require('../lib/prisma');
const CHUNK = 2000;
async function run() {
  const oldIds = (await prisma.station.findMany({ where: { country: 'GR' }, select: { id: true } })).map(s => s.id);
  if (oldIds.length) {
    ...
    console.log(`[script] Deleted ${oldIds.length} old GR stations`);
  }
  const stations = await fetchGreeceStations();
  console.log(`[script] Fetched ${stations.length} Greece stations`);
  let totalPrices = 0;
