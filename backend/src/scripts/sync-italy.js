...
const { fetchItalyStations } = require('../services/scrapers/italy');
const prisma = require('../lib/prisma');
const CHUNK = 2000;
async function run() {
  const oldIds = (await prisma.station.findMany({ where: { country: 'IT' }, select: { id: true } })).map(s => s.id);
  if (oldIds.length) {
    ...
    console.log(`[script] Deleted ${oldIds.length} old IT stations`);
  }
  const stations = await fetchItalyStations();
  console.log(`[script] Fetched ${stations.length} Italy stations`);
  let totalPrices = 0;
