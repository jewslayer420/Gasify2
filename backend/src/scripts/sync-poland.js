...
const { fetchPolandStations } = require('../services/scrapers/poland');
const prisma = require('../lib/prisma');

const CHUNK = 2000;

async function run() {
  console.log('[script] Clearing old PL stations...');
  const oldIds = (await prisma.station.findMany({ where: { country: 'PL' }, select: { id: true } })).map(s => s.id);
  if (oldIds.length) {
    ...
    console.log(`[script] Deleted ${oldIds.length} old PL stations`);
  }

  const stations = await fetchPolandStations();
  console.log(`[script] Fetched ${stations.length} Poland stations`);
