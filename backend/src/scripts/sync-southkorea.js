...
const { fetchSouthKoreaStations } = require('../services/scrapers/southkorea');
const prisma = require('../lib/prisma');

const CHUNK = 500;

async function run() {
  const stations = await fetchSouthKoreaStations();
  console.log(`[script] Fetched ${stations.length} South Korea stations`);
