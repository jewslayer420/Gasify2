...
const { fetchTurkeyStations } = require('../services/scrapers/turkey');
const prisma = require('../lib/prisma');

const CHUNK = 500;

async function run() {
  const existing = await prisma.station.findMany({ where: { country: 'TR' }, select: { id: true } });
  ...
  if (oldIds.length) {
    ...
    console.log(`[script] Deleted ${oldIds.length} old TR stations`);
  }

  const stations = await fetchTurkeyStations();
  console.log(`[script] Fetched ${stations.length} Turkey stations`);
