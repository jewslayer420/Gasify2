...
const { fetchBelgiumStations } = require('../services/scrapers/belgium');
...
async function run() {
  ...
  console.log(`[script] ${coordsCache.size} cached BE station coords`);
  ...
  if (oldIds.length) {
    ...
    console.log(`[script] Deleted ${oldIds.length} old BE stations`);
  }

  const stations = await fetchBelgiumStations(coordsCache);
  console.log(`[script] Fetched ${stations.length} Belgium stations`);
