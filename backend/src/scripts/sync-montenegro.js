...
const { fetchMontenegroStations } = require('../services/scrapers/montenegro');
const prisma = require('../lib/prisma');
const CHUNK = 2000;
async function run() {
  const oldIds = (await prisma.station.findMany({ where: { country: 'ME' }, select: { id: true } })).map(s => s.id);
  if (oldIds.length) {
    ...
    console.log(`[script] Deleted ${oldIds.length} old ME stations`);
  }
  const stations = await fetchMontenegroStations();
  console.log(`[script] Fetched ${stations.length} Montenegro stations`);
  let totalPrices = 0;
