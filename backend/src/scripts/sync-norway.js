...
const { fetchNorwayStations } = require('../services/scrapers/norway');
...
async function run() {
  ...
  console.log(`[script] ${coordsCache.size} cached NO station coords`);
  ...
  if (oldIds.length) {
    ...
    console.log(`[script] Deleted ${oldIds.length} old NO stations`);
  }

  const stations = await fetchNorwayStations(coordsCache);
  console.log(`[script] Fetched ${stations.length} Norway stations`);
