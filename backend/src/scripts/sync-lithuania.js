...
const { fetchLithuaniaStations } = require('../services/scrapers/lithuania');
const prisma = require('../lib/prisma');

const CHUNK = 500;

async function run() {
  const existing = await prisma.station.findMany({ where: { country: 'LT' }, select: { id: true } });
  ...
  if (oldIds.length) {
    ...
    console.log(`[script] Deleted ${oldIds.length} old LT stations`);
  }

  const stations = await fetchLithuaniaStations();
  console.log(`[script] Fetched ${stations.length} Lithuania stations`);
