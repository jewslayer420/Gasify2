...
const { fetchEstoniaStations } = require('../services/scrapers/estonia');
const prisma = require('../lib/prisma');

const CHUNK = 500;

async function run() {
  const existing = await prisma.station.findMany({ where: { country: 'EE' }, select: { id: true } });
  ...
  if (oldIds.length) {
    ...
    console.log(`[script] Deleted ${oldIds.length} old EE stations`);
  }

  const stations = await fetchEstoniaStations();
  console.log(`[script] Fetched ${stations.length} Estonia stations`);
