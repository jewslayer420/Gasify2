// Greece fuel prices via gr.fuelo.net — no API key needed
const PHASE1_URL = 'https://gr.fuelo.net/ajax/get_gasstations_within_bounds_mysql_clustering';
const PHASE2_BASE = 'https://gr.fuelo.net/ajax/get_infowindow_content';
const GRID_STEP = 0.25;
const BOUNDS = { latMin: 34.8, latMax: 42.0, lngMin: 19.3, lngMax: 29.7 };

const COUNTRY_NAME_MAP = {
  greece: 'GR', hellas: 'GR', griechenland: 'GR', 'ελλάδα': 'GR', ellada: 'GR', grecia: 'GR',
  bulgaria: 'BG', 'north macedonia': 'MK', macedonia: 'MK',
  albania: 'AL', shqiperia: 'AL', turkey: 'TR', türkiye: 'TR',
};
...
function mapFuelType(name) {
  ...
  if (n.includes('lpg') || n.includes('autogas') || n.includes('υγρα') || n.includes('υγραέριο')) return 'lpg';
  if (n.includes('cng') || n.includes('φυσικο')) return 'cng';
  if (n.includes('e10')) return 'e10';
  const isDiesel = n.includes('diesel') || n.includes('πετρελαιο') || n.includes('πετρέλαιο');
  const isPremium = n.includes('premium') || n.includes('plus') || n.includes('ultimate') || n.includes('v-power') || n.includes('excellium');
  if (isDiesel && isPremium) return 'diesel_premium';
  if (isDiesel) return 'diesel';
  if (n.includes('98') || n.includes('100')) return 'sp98';
  if (n.includes('95') || n.includes('super') || n.includes('unleaded') || n.includes('αμολυβδη') || n.includes('αμόλυβδη') || n.includes('e5')) return 'sp95';
  return null;
}
...
async function fetchDetail(id, coords) {
  try {
    ...
    const detectedCountry = COUNTRY_NAME_MAP[parts[0]?.toLowerCase()] ?? 'GR';
    if (detectedCountry !== 'GR') return null;
    const prices = parsePrices(html);
    if (!prices.length) return null;
    return {
      externalId: `GR-${id}`, name: nameMatch ? nameMatch[1].trim() : `Station ${id}`,
...
async function fetchGreeceStations() {
  ...
  await runConcurrent(cells, async (c) => {
    ...
    if (++done % 20 === 0) console.log(`[greece] Phase 1: ${done}/${cells.length} cells, ${stationIdMap.size} stations`);
  });
  console.log(`[greece] Phase 1 done — ${stationIdMap.size} unique station IDs`);
  const stations = [];
  ...
  await runConcurrent(ids, async ([id, coords]) => {
    ...
    if (++i % 100 === 0) console.log(`[greece] Phase 2: ${i}/${ids.length}, ${stations.length} GR stations`);
  });
  console.log(`[greece] Done — ${stations.length} stations`);
  return stations;
}

module.exports = { fetchGreeceStations };
