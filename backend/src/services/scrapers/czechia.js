// Czech Republic fuel prices via fuelo.net public AJAX endpoints — no API key needed
...
function mapFuelType(name) {
  ...
  if (n.includes('95') || n.includes('unleaded') || n.includes('natural') || n.includes('benzin') || n.includes('super') || n.includes('efecta') || n.includes('e5')) return 'sp95';
  return null;
}
...
function parsePrices(html) {
  ...
  while ((m = regex.exec(html)) !== null) {
    ...
    const price = currency === 'CZK' ? +(raw / CZK_EUR).toFixed(3) : +raw.toFixed(3);
...
async function fetchCzechiaStations() {
  ...
  await runConcurrent(cells, async (c) => {
    ...
    if (done % 20 === 0) console.log(`[czechia] Phase 1: ${done}/${cells.length} cells, ${stationIdMap.size} stations`);
  });
  console.log(`[czechia] Phase 1 done — ${stationIdMap.size} unique station IDs`);
  ...
  await runConcurrent(ids, async ([id, coords]) => {
    ...
    i++;
    if (i % 100 === 0) console.log(`[czechia] Phase 2: ${i}/${ids.length}, ${stations.length} with prices`);
  });

  console.log(`[czechia] Done — ${stations.length} stations`);
  return stations;
}
