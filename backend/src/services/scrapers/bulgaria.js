// Bulgaria fuel prices via bg.fuelo.net — no API key needed
const PHASE1_URL = 'https://bg.fuelo.net/ajax/get_gasstations_within_bounds_mysql_clustering';
const PHASE2_BASE = 'https://bg.fuelo.net/ajax/get_infowindow_content';
const GRID_STEP = 0.2;
const BOUNDS = { latMin: 41.2, latMax: 44.2, lngMin: 22.4, lngMax: 28.7 };

const COUNTRY_NAME_MAP = {
  bulgaria: 'BG', balgaria: 'BG', bulgarien: 'BG', 'българия': 'BG',
  romania: 'RO', serbia: 'RS', srbija: 'RS',
  'north macedonia': 'MK', macedonia: 'MK', makedonija: 'MK',
  greece: 'GR', hellas: 'GR', türkiye: 'TR', turkey: 'TR',
};
...
function mapFuelType(name) {
  ...
  if (n.includes('cng') || n.includes('gnv') || n.includes('methane')) return 'cng';
  if (n.includes('e10')) return 'e10';
  const isDiesel = n.includes('diesel') || n.includes('дизел');
  ...
  if (n.includes('95') || n.includes('super') || n.includes('unleaded') || n.includes('е5') || n.includes('e5') || n.includes('a95')) return 'sp95';
  return null;
}
...
async function fetchDetail(id, coords) {
  try {
    ...
    const detectedCountry = COUNTRY_NAME_MAP[parts[0]?.toLowerCase()] ?? 'BG';
    if (detectedCountry !== 'BG') return null;
    const prices = parsePrices(html);
    if (!prices.length) return null;
    return {
      externalId: `BG-${id}`, name: nameMatch ? nameMatch[1].trim() : `Station ${id}`,
...
async function fetchBulgariaStations() {
  ...
  await runConcurrent(cells, async (c) => {
    ...
    if (++done % 20 === 0) console.log(`[bulgaria] Phase 1: ${done}/${cells.length} cells, ${stationIdMap.size} stations`);
  });
  console.log(`[bulgaria] Phase 1 done — ${stationIdMap.size} unique station IDs`);
  const stations = [];
  ...
  await runConcurrent(ids, async ([id, coords]) => {
    ...
    if (++i % 100 === 0) console.log(`[bulgaria] Phase 2: ${i}/${ids.length}, ${stations.length} BG stations`);
  });
  console.log(`[bulgaria] Done — ${stations.length} stations`);
  return stations;
}

module.exports = { fetchBulgariaStations };
