...
const COUNTRY_NAME_MAP = {
  montenegro: 'ME', 'crna gora': 'ME', crnagora: 'ME', monténégro: 'ME', crna: 'ME',
  serbia: 'RS', srbija: 'RS',
  'bosnia and herzegovina': 'BA', 'bosna i hercegovina': 'BA', bosna: 'BA',
...
function mapFuelType(name) {
  ...
  const isDiesel = n.includes('diesel') || n.includes('dizel');
...
function parsePrices(html) {
  ...
  while ((m = regex.exec(html)) !== null) {
    ...
    if (currency === 'BAM') raw = raw / 1.9558; // Bosnian Mark → EUR (just in case)
...
async function fetchDetail(id, coords) {
  try {
    ...
    const detectedCountry = COUNTRY_NAME_MAP[parts[0]?.toLowerCase()] ?? 'ME';
    if (detectedCountry !== 'ME') return null;
    const prices = parsePrices(html);
    if (!prices.length) return null;
    return {
      externalId: `ME-${id}`, name: nameMatch ? nameMatch[1].trim() : `Station ${id}`,
...
async function fetchMontenegroStations() {
  ...
  await runConcurrent(cells, async (c) => {
    ...
    if (++done % 20 === 0) console.log(`[montenegro] Phase 1: ${done}/${cells.length} cells, ${stationIdMap.size} stations`);
  });
  console.log(`[montenegro] Phase 1 done — ${stationIdMap.size} unique station IDs`);
  const stations = [];
  ...
  await runConcurrent(ids, async ([id, coords]) => {
    ...
    if (++i % 50 === 0) console.log(`[montenegro] Phase 2: ${i}/${ids.length}, ${stations.length} ME stations`);
  });
  console.log(`[montenegro] Done — ${stations.length} stations`);
  return stations;
}

module.exports = { fetchMontenegroStations };
