// Bosnia and Herzegovina fuel prices via ba.fuelo.net — no API key needed
const PHASE1_URL = 'https://ba.fuelo.net/ajax/get_gasstations_within_bounds_mysql_clustering';
const PHASE2_BASE = 'https://ba.fuelo.net/ajax/get_infowindow_content';
const GRID_STEP = 0.15;
const BOUNDS = { latMin: 42.5, latMax: 45.3, lngMin: 15.7, lngMax: 19.7 };

const COUNTRY_NAME_MAP = {
  'bosnia and herzegovina': 'BA', 'bosna i hercegovina': 'BA', bosna: 'BA', bih: 'BA',
  'b&h': 'BA', hercegovina: 'BA', 'republika srpska': 'BA',
  'federacija bosne i hercegovine': 'BA', 'federacija bih': 'BA', bosnien: 'BA',
...
function mapFuelType(name) {
  ...
  const isDiesel = n.includes('diesel') || n.includes('dizel') || n.includes('gasoil');
  ...
  if (n.includes('95') || n.includes('super') || n.includes('eurosuper') || n.includes('unleaded') || n.includes('e5')) return 'sp95';
  return null;
}
...
async function fetchDetail(id, coords) {
  try {
    ...
    const detectedCountry = COUNTRY_NAME_MAP[parts[0]?.toLowerCase()] ?? 'BA';
    if (detectedCountry !== 'BA') return null;
    const prices = parsePrices(html);
    if (!prices.length) return null;
    return {
      externalId: `BA-${id}`, name: nameMatch ? nameMatch[1].trim() : `Station ${id}`,
...
async function fetchBosniaStations() {
  ...
  await runConcurrent(cells, async (c) => {
    ...
    if (++done % 20 === 0) console.log(`[bosnia] Phase 1: ${done}/${cells.length} cells, ${stationIdMap.size} stations`);
  });
  console.log(`[bosnia] Phase 1 done — ${stationIdMap.size} unique station IDs`);
  const stations = [];
  ...
  await runConcurrent(ids, async ([id, coords]) => {
    ...
    if (++i % 100 === 0) console.log(`[bosnia] Phase 2: ${i}/${ids.length}, ${stations.length} BA stations`);
  });
  console.log(`[bosnia] Done — ${stations.length} stations`);
  return stations;
}

module.exports = { fetchBosniaStations };
