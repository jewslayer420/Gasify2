// Croatia fuel prices via hr.fuelo.net public AJAX endpoints — no API key needed
const PHASE1_URL = 'https://hr.fuelo.net/ajax/get_gasstations_within_bounds_mysql_clustering';
const PHASE2_BASE = 'https://hr.fuelo.net/ajax/get_infowindow_content';
const GRID_STEP = 0.2;
const BOUNDS = { latMin: 42.40, latMax: 46.60, lngMin: 13.50, lngMax: 19.50 };
...
function mapFuelType(name) {
  ...
  const isDiesel = n.includes('diesel') || n.includes('dizel') || n.includes('gazole');
  ...
  if (n.includes('95') || n.includes('eurosuper') || n.includes('super') || n.includes('unleaded') || n.includes('e5')) return 'sp95';
  return null;
}
...
function parsePrices(html) {
  ...
  while ((m = regex.exec(html)) !== null) {
    ...
    const price = cur === 'EUR' ? +raw.toFixed(3) : +raw.toFixed(3);
...
async function fetchDetail(id, coords) {
  try {
    ...
    const detectedCountry = COUNTRY_NAME_MAP[parts[0]?.toLowerCase()] ?? 'HR';
    if (detectedCountry !== 'HR') return null;
...
async function fetchCroatiaStations() {
  ...
  await runConcurrent(cells, async (c) => {
    ...
    if (++done % 20 === 0) console.log(`[croatia] Phase 1: ${done}/${cells.length} cells, ${stationIdMap.size} stations`);
  });
  console.log(`[croatia] Phase 1 done — ${stationIdMap.size} unique station IDs`);
  ...
  await runConcurrent(ids, async ([id, coords]) => {
    ...
    if (++i % 100 === 0) console.log(`[croatia] Phase 2: ${i}/${ids.length}, ${stations.length} HR stations`);
  });
  console.log(`[croatia] Done — ${stations.length} stations`);
  return stations;
}

module.exports = { fetchCroatiaStations };
