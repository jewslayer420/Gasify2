...
const COUNTRY_NAME_MAP = {
  ...
  'bosnia and herzegovina': 'BA', bosna: 'BA',
  bulgaria: 'BG', montenegro: 'ME',
};

// RSD → EUR (approx 1 EUR = 117 RSD)
const RSD_EUR = 117;
...
function mapFuelType(name) {
  ...
  if (n.includes('lpg') || n.includes('autogas') || n.includes('tng')) return 'lpg';
  ...
  if (n.includes('95') || n.includes('ekonomy') || n.includes('super') || n.includes('unleaded') || n.includes('e5')) return 'sp95';
  return null;
}

function convertPrice(raw, cur) {
  switch (cur) {
    case 'RSD': return +(raw / RSD_EUR).toFixed(3);
...
async function fetchDetail(id, coords) {
  try {
    ...
    const detectedCountry = COUNTRY_NAME_MAP[parts[0]?.toLowerCase()] ?? 'RS';
    if (detectedCountry !== 'RS') return null;
...
async function fetchSerbiaStations() {
  ...
  await runConcurrent(cells, async (c) => {
    ...
    if (++done % 20 === 0) console.log(`[serbia] Phase 1: ${done}/${cells.length} cells, ${stationIdMap.size} stations`);
  });
  console.log(`[serbia] Phase 1 done — ${stationIdMap.size} unique station IDs`);
  ...
  await runConcurrent(ids, async ([id, coords]) => {
    ...
    if (++i % 100 === 0) console.log(`[serbia] Phase 2: ${i}/${ids.length}, ${stations.length} RS stations`);
  });
  console.log(`[serbia] Done — ${stations.length} stations`);
  return stations;
}

module.exports = { fetchSerbiaStations };
