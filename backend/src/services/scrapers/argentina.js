...
//
// NOTE: the host (datos.energia.gob.ar) is unreachable from some networks (incl. dev sandboxes)
//   but IS reachable from the Render production IP. On failure this returns [] harmlessly.
...
async function fetchArgentinaStations() {
  ...
  // One station = one (idempresa + lat/lng). externalId is derived 1:1 from that same key,
  // so two distinct stations can never collide on externalId (avoids a price-insert crash).
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const idPart = (iId !== -1 ? String(f[iId]).trim() : '') || 'x';
    const key = `${idPart}|${lat.toFixed(5)}|${lng.toFixed(5)}`;
    if (!stationMap.has(key)) {
      stationMap.set(key, {
        externalId: `AR-${idPart}-${lat.toFixed(5)}_${lng.toFixed(5)}`,
