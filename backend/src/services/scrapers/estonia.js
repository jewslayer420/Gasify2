// Estonia fuel prices via ee.fuelo.net — no API key needed
const PHASE1_URL = 'https://ee.fuelo.net/ajax/get_gasstations_within_bounds_mysql_clustering';
const PHASE2_BASE = 'https://ee.fuelo.net/ajax/get_infowindow_content';
const GRID_STEP = 0.15;
const BOUNDS = { latMin: 57.5, latMax: 59.8, lngMin: 21.8, lngMax: 28.3 };
...
function mapFuelType(name) {
  ...
  if (n.includes('lpg') || n.includes('autogas') || n.includes('autogaas')) return 'lpg';
  if (n.includes('cng')) return 'cng';
  if (n.includes('e10')) return 'e10';
  const isDiesel = n.includes('diesel') || n.includes('diislikütus');
  ...
  if (n.includes('95') || n.includes('unleaded') || n.includes('super') || n.includes('bensiin')) return 'sp95';
  return null;
}
...
async function fetchCell(latMin, latMax, lngMin, lngMax, stationIdMap) {
  ...
  try {
    const res = await fetch(PHASE1_URL, {
      method: 'POST',
      headers: { 'Content-Type': 'application/x-www-form-urlencoded', 'User-Agent': UA, 'Referer': 'https://ee.fuelo.net/' },
...
async function fetchDetail(id, coords) {
  try {
    ...
    if (parts[0]?.toLowerCase() !== 'estonia') return null;
...
async function fetchEstoniaStations() {
  ...
  await runConcurrent(cells, async (c) => {
    ...
    if (++done % 50 === 0) console.log(`[estonia] Phase 1: ${done}/${cells.length} cells, ${stationIdMap.size} stations`);
  });
  console.log(`[estonia] Phase 1 done — ${stationIdMap.size} unique station IDs`);
  ...
  await runConcurrent(ids, async ([id, coords]) => {
    ...
    if (++i % 50 === 0) console.log(`[estonia] Phase 2: ${i}/${ids.length}, ${stations.length} EE stations`);
  });
  console.log(`[estonia] Done — ${stations.length} stations`);
  return stations;
}

module.exports = { fetchEstoniaStations };
