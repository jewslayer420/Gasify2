...
Key backend paths: `src/services/scrapers/` (per-country price scrapers), `src/services/sync.js` (bulkUpsertStations + scheduling), `src/routes/stations.js` (API incl. gzipped `/geojson` the frontend actually uses — the 2026-07-18 grid/bbox "map diet" was reverted same day (owner: heatmap looked chopped when zoomed); `/overview`, sorted bbox and `country=` remain server-side, unused by the map), `src/scripts/sync_all_now.js` (fast|slow|all runner), `src/scripts/check_freshness.js` (monitor, `--dry-run`).

## Commands

```bash
# backend tests (node:test — run explicit files, `node --test src` misbehaves on Windows)
cd backend && node --test src/middleware/rateLimit.test.js src/services/scrapers/_overpass.test.js src/services/scrapers/thailand.test.js src/services/freshness_monitor.test.js src/services/telegram.test.js src/services/price_alerts.test.js
